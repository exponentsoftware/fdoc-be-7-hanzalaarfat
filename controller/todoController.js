...
exports.getalltodo = async (req, res) => {
  ...
  if (!todo) {
    res.status(200).json({ success: false, message: "Not Found any Todo" });
  }
  res.status(200).json({ success: true, message: "All User's todo", todo });
...
exports.getAllTodoById = async (req, res) => {
  ...
  const { page = 1, limit = 1 } = req.query; ///////////Pagination
